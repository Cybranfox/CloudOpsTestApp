#!/bin/bash
# 🔧 fix_lesson_circles.sh - Fix the lesson display to show proper circular buttons

echo "🔧 CloudQuest RPG - Fix Lesson Circle Display"
echo "============================================="

echo "🎯 Fixing lesson circles to display properly..."
...
{% block head %}
<style>
/* Duolingo-Inspired Sci-Fi Learning Map - Fixed Circles */
...
/* Individual lesson circles - FIXED STYLING */
.lesson-circle {
    width: 90px;
    height: 90px;
    border-radius: 50%;
...
/* Lesson states with proper sci-fi colors */
.lesson-circle.completed {
    background: linear-gradient(135deg, #06d6a0, #059669);
    color: white;
    border-color: rgba(6, 214, 160, 0.4);
    box-shadow: 
        0 8px 20px rgba(6, 214, 160, 0.3),
        0 0 30px rgba(6, 214, 160, 0.2),
        inset 0 2px 0 rgba(255, 255, 255, 0.3);
}

.lesson-circle.completed::before {
    content: '✓';
    position: absolute;
    font-size: 32px;
    font-weight: 900;
...
@keyframes currentPulse {
    0%, 100% { 
        transform: scale(1);
        box-shadow: 0 8px 20px rgba(245, 158, 11, 0.4), 0 0 35px rgba(245, 158, 11, 0.3);
    }
    50% { 
        transform: scale(1.1);
        box-shadow: 0 10px 25px rgba(245, 158, 11, 0.6), 0 0 45px rgba(245, 158, 11, 0.5);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🎮 Fixed Duolingo-Style Sci-Fi Map - Ready!');
});
</script>
{% endblock %}
EOF

echo "✅ Lesson circles fixed and properly styled!"

echo ""
echo "🔧 Lesson Circle Display Fixed!"
echo "==============================="
echo ""
echo "🎯 FIXES APPLIED:"
echo "• Proper circular button styling (90px diameter)"
echo "• Clear lesson number display inside circles"
echo "• Enhanced gradients and shadows"
echo "• Better hover and click effects"
echo "• Improved zigzag pattern layout"
echo "• Mobile-responsive circle sizing"
echo ""
echo "🚀 Refresh your browser to see the fixed circles!"